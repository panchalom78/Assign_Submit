...
const AffiliationForm = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5134/api/auth/colleges", { withCredentials: true })
  ...
  const fetchFaculties = (collegeId) => {
    axios
      .get(`http://localhost:5134/api/auth/faculties?collegeId=${collegeId}`, {
  ...
  const fetchCourses = (facultyId) => {
    axios
      .get(`http://localhost:5134/api/auth/courses?facultyId=${facultyId}`, {
        withCredentials: true,
      })
  ...
  const fetchClasses = (courseId) => {
    axios
      .get(`http://localhost:5134/api/auth/classes?courseId=${courseId}`, {
        withCredentials: true,
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setError(error.response?.data?.error || "Bad request. Please check your selections.");
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-50 to-purple-50 flex items-center justify-center p-6">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Select Affiliation</h2>
